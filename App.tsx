...
export default function App() {
  ...
  const addCustomerJob = async (data: Omit<CustomerJob, 'id'>) => {
    ...
    try {
      ...
      if (validMaterials.length > 0) {
        ...
        await apiRequest(`/Musteriler/isler/${saved.id}/malzemeler/bulk`, 'POST', validMaterials);
  ...
  const updateCustomerJob = async (data: CustomerJob) => {
    ...
    const saved = await apiRequest(`/Musteriler/isler/${data.id}`, 'PUT', body);
    ...
    try {
      ...
      if (validMaterials.length > 0) {
        // Bulk endpoint kullan - sadece geçerli malzemeleri gönder
        await apiRequest(`/Musteriler/isler/${data.id}/malzemeler/bulk`, 'POST', validMaterials);
      }
    } catch (e) {
